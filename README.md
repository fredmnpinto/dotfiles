...
2. Add the new config path to `local_paths` in `./setup.sh`. The key is the name of the folder/file while the value is the path the config should have in the system, e.g. nvim `local_paths[nvim]="$HOME/.config/nvim"`.

**Example**:
```bash
# [...]
local_paths[nvim] = "/path/to/nvim"
local_paths[another_config] = "/path/to/config"
# [...]
```
```
